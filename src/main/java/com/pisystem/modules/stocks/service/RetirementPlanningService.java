...
public class RetirementPlanningService {
    ...
    public Map<String, Object> calculateRetirementPlan(
        ...
        // Future value of monthly contributions (annuity): FV = PMT * [((1+r)^n - 1) /
        // r]
        ...
        BigDecimal inflationFactor = BigDecimal.ONE.add(
                inflationRate.divide(BigDecimal.valueOf(100), 6, RoundingMode.HALF_UP)).pow(yearsToRetirement);
        ...
        result.put("recommendations",
                generateRetirementRecommendations(shortfall, additionalContribution, yearsToRetirement));

        return result;
    }

    public Map<String, Object> calculateRetirementPlanForUser(
            Long userId,
            BigDecimal currentSavings,
            BigDecimal monthlyContribution,
            BigDecimal expectedReturn,
        ...
        int currentAge = (profile.getAge() != null && profile.getAge() > 0) ? profile.getAge() : 30;
        int retirementAge = profile.getRetirementAge() != null ? profile.getRetirementAge() : 60;
        int lifeExpectancy = profile.getLifeExpectancy() != null ? profile.getLifeExpectancy() : 80;

        // Use 70% of current effective monthly income as desired monthly income in
        // retirement if not provided
        BigDecimal desiredMonthlyIncome = profile.getEffectiveMonthlyIncome().multiply(new BigDecimal("0.7"));
        if (desiredMonthlyIncome.compareTo(BigDecimal.ZERO) == 0) {
            desiredMonthlyIncome = new BigDecimal("50000"); // Fallback
        }
    ...
    private List<String> generateRetirementRecommendations(BigDecimal shortfall, BigDecimal additionalContribution,
            int yearsToRetirement) {
        List<String> recommendations = new ArrayList<>();

        if (shortfall.compareTo(BigDecimal.ZERO) > 0) {
            recommendations
                    .add("Increase monthly contributions by ₹" + additionalContribution + " to meet retirement goal");
        ...
        if (yearsToRetirement > 20) {
            recommendations.add(
                    "With " + yearsToRetirement + " years to retirement, consider more aggressive growth investments");
        } else if (yearsToRetirement < 10) {
            recommendations
                    .add("With less than 10 years to retirement, consider shifting to more conservative investments");
        }
    ...
    public Map<String, Object> calculateSafWithdrawalRate(BigDecimal portfolioValue, int withdrawalYears) {
        ...
        BigDecimal annualWithdrawal = portfolioValue
                .multiply(safeWithdrawalRate.divide(BigDecimal.valueOf(100), 4, RoundingMode.HALF_UP));
